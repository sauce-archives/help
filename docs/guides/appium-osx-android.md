...
_TODO: insert screenshots with filled-in fields?_
...
_TODO: complete branch-off for the Inspector since it differs a lot in different OSs?_

At the time of writing this tutorial, the Appium Inspector differs a lot in look and feel from OS to OS. While the OS X version is easier on the eye, the one on Windows may look confusing to a new user. There currently is no Inspector available for Linux, so a solution would be using the UIAutomator Viewer that comes with the Android SDK.

Using either the Inspector or the UIAutomator you will be able to reference the UI elements of the app you are testing so that you can interact with them.
...
Next, open up Eclipse and create a new project. In this project, create a new JUnit class. This is where we will be writing our tests.

_TODO: insert blank class screenshot_

Now you must import the jars we previously downloaded into the project. To do this just right click on you project in the Package Explorer and select _Properties_, then select _Java Build Path_ and then click on _Add Eternal JARs_. Now select the Appium Java Client and the Selenium Server JARs and add them to the project.
